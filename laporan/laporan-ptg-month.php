...
$pdf->SetTextColor(0);
$i=0;
$tampil=mysqli_query($conn,"SELECT * FROM transaksi WHERE month(tanggal)=month(curdate()) AND year(tanggal)=year(curdate()) AND status = 'BELUM LUNAS' OR month(tanggal)=month(curdate()) AND year(tanggal)=year(curdate()) AND status = 'PINJAMAN' order by tanggal DESC");
